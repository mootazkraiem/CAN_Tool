...
import numpy as np
from .theme import CARD_MARGIN, COMPACT_SPACING, PAGE_MARGIN, SECTION_SPACING
...
class AnalyticsWidget(QWidget):
    def __init__(self, parent=None):
        ...
        tile_data = [
            ("TOTAL MESSAGES", "1,245,802", "MSG", "#8fefff"),
            ("BUS UTILIZATION", "42.8", "%", "#c8b5ff"),
            ("ERROR FRAMES", "14", "ERR", "#ffb45c"),
            ("PEAK BITRATE", "501.2", "kbps", "#8fefff"),
        ]

        for title, val, unit, color in tile_data:
            tile = QFrame()
            tile.setObjectName("SurfaceCard")
            l = QVBoxLayout(tile)
        ...
        for title, accent in [("BUS LOAD TREND", "#8fefff"), ("RESPONSE LATENCY", "#c8b5ff")]:
            c_panel = QFrame()
            ...
            for _ in range(30):
                ...
                b.setFixedWidth(6)
                base = "#252933" if h_val < 60 else accent
        ...
        log_lay.setContentsMargins(CARD_MARGIN, CARD_MARGIN, CARD_MARGIN, CARD_MARGIN)
